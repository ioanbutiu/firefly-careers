...
// for the fixed text overlay.
function buildCollage(): CollageItem[] {
  const rng = mulberry32(7);
  const items: CollageItem[] = [];
  const SX = 200; // half-width of the cleared center (text safe-zone)
  const SY = 140; // half-height of the cleared center
  let i = 0;
  for (let gx = -1500; gx <= 1500; gx += 420) {
    for (let gy = -840; gy <= 840; gy += 320) {
      const cx = gx + (rng() - 0.5) * 260; // jitter off the grid
      const cy = gy + (rng() - 0.5) * 260;
      const w = 260; // uniform size — only position & rotation vary
      const half = w / 2;
      // skip any card that would intrude on the center text
      if (Math.abs(cx) < SX + half && Math.abs(cy) < SY + half) continue;
      items.push({
